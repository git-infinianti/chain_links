...
class DefaultPath:
    def __init__(self, chain: str) -> None:
        self.chain = chain
        self.capChain = chain.capitalize()
    ...
    def get_default_path(self):
        ...
        if operating_system is platform[0]:
            return join(home, f'.{self.chain}')
        elif operating_system is platform[1]:
            return join(home, 'Library', 'Application', 'Support', self.capChain)
        return join(home, 'AppData', 'Roaming', self.capChain)


def generate_conf(
    chain, testnet=None, regtest=None, proxy=None, bind=None, whitebind=None, nodes=None, connections=None, listen=1, maxconnections=None, 
    server=1, rpcbinds=None, authentications=None, rpcuser='user', rpcpassword='pass', rpcclienttimeout=30, rpcallowedips=['127.0.0.1'],
    zmqpub=None, rpcport=8766, wallet=None, datadir=None, rpcconnect='127.0.0.1', txconfirmtarget=6, keypool=100, paytxfee=None,
    prune=0, minimized=0, totray=1, txindex=1, addressindex=1, assetindex=1, timestampindex=1, spentindex=1 
):
    ws = ' '
    hl = '#' * 62
    ...
    setup = DefaultPath(chain)
    capChain: str = setup.capChain
    default_path = datadir if datadir else setup.get_default_path()
    ...
    with open(join(default_path, f'{chain}.conf'), 'w') as file:
        settings = [
            ...
            *connect, '\n', '# Listening mode, enabled by default except when \'connect\' is being used\n',
            ...
            maxconnections, '\n', '#\n', f'# JSON-RPC options (for controlling a running {capChain}/{chain}d process)\n',
            '#\n', '\n', f'# server=1 tells {capChain}-Qt and {chain}d to accept JSON-RPC commands\n', f'server={server}\n',
